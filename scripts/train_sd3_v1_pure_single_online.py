...
def get_logger(name):
    # 轻量 logger（避免 accelerate.logger）
    import logging
...
def save_ckpt_1gpu(save_dir, transformer, global_step, ema, transformer_trainable_parameters, config):
    save_root = os.path.join(save_dir, "checkpoints", f"checkpoint-{global_step}")
    save_root_lora = os.path.join(save_root, "lora")
    os.makedirs(save_root_lora, exist_ok=True)
    if config.train.ema:
        ema.copy_ema_to(transformer_trainable_parameters, store_temp=True)
    transformer.save_pretrained(save_root_lora)
    if config.train.ema:
        ema.copy_temp_to(transformer_trainable_parameters)
...
def save_reward_model(reward_model, global_step, config):
    ...
    reward_model.save_pretrained(save_root)
...
def main(_):
    ...
    wandb.init(project="flow_grpo_v1_nodist_singleGPU_rm_update", mode="offline")
    ...
    # more memory
    # AMP & autocast
    use_amp = (device.type == "cuda") and (config.mixed_precision in ["fp16", "bf16"])
    amp_dtype = torch.float16 if config.mixed_precision == "fp16" else torch.float32
    scaler = torch.cuda.amp.GradScaler(enabled=(use_amp and config.mixed_precision == "fp16"))
    if config.use_lora:
        autocast_ctx = contextlib.nullcontext  # 与你的原逻辑一致
    else:
        def _autocast():
    ...
    # Train!
    samples_per_epoch = config.sample.train_batch_size * 1 * config.sample.num_batches_per_epoch
    total_train_batch_size = config.train.batch_size * 1 * config.train.gradient_accumulation_steps
    ...
    while True:
        ...
        if epoch % config.save_freq == 0 and epoch > 0 :
            save_ckpt_1gpu(config.save_dir, transformer, global_step, ema, transformer_trainable_parameters, config)
        ...
        gathered_rewards = {key: torch.cat([value]) for key, value in samples["rewards"].items()}
        ...
        if config.per_prompt_stat_tracking:
            ...
            prompt_ids = torch.cat([samples["prompt_ids"]]).cpu().numpy()
        ...
        samples["advantages"] = (
            advantages.reshape(1, -1, advantages.shape[-1])[0]
            .to(device)
        )
                ...
                for j in tqdm(
                    ...
                    if True:
                        ...
                        if microstep % global_accum_steps == 0:
                            if config.train.max_grad_norm is not None and config.train.max_grad_norm > 0:
                                if scaler.is_enabled():
        ...
        if config.train.update_reward_model:
            ...
            if len(positive_images_batched) > 0:
                print("lengths of positive_images_batched: ", len(positive_images_batched))
                ...
                print("reward_optimizer.step(), the contrastive loss is: ", contrastive_loss)
                reward_optimizer.zero_grad()
                wandb.log({"reward_contrastive_loss": contrastive_loss.item()}, step=reward_step)
                reward_step += 1
