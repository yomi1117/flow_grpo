...
def unwrap_fsdp(m):
    ...
    if hasattr(m, "module"):  # e.g., DDP
        m = m.module
    return m  # FSDP2 的 FSDPModule 本身可用；命名/参数枚举都代理到原模块
...
def compute_contrastive_loss(clip_model, clip_processor, positive_images, negative_images, positive_prompts, negative_prompts, device):
    ...
    def process_images(images):
        images_np = (images.detach().cpu().numpy() * 255).astype(np.uint8)
...
def main(_):
    ...
    if config.train.update_reward_model:
        ...
        reward_model = reward_fn.score_fns[reward_key[0]].model
        reward_processor = reward_fn.score_fns[reward_key[0]].processor
    ...
    pipeline.transformer = transformer  # 重要：让 pipeline 使用 FSDP 包裹后的模块
    ...
    while True:
        ...
        # ---------- SAMPLING ----------
        unwrap_fsdp(transformer).eval()  # eval/ train 调 FSDP 包裹对象本身也 OK，这里用 unwrap 保守
        if config.train.update_reward_model:
            unwrap_fsdp(reward_model).eval()

        samples = []
        last_local_images = None  # 用于 rank0 可视化（各 rank 自己的）
        for i in tqdm(
        ...
        print("samples' prompts:", [(i, samples[i]['prompts']) for i in range(len(samples))])
        # 等所有 reward 完成，并转成 tensor
        for s in tqdm(samples, desc="Waiting for rewards", disable=False, position=0):
        ...
        # rank0 可视化（仅本 rank 的图片也够看）
        if (epoch % 10 == 0) and is_main_process():
            with tempfile.TemporaryDirectory() as tmpdir:
                imgs = last_local_images
                num_samples = min(15, len(imgs))
                idxs = random.sample(range(len(imgs)), num_samples)
                for idx, ii in enumerate(idxs):
                    image = imgs[ii]
                    pil = Image.fromarray((image.cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8))
                    pil = pil.resize((config.resolution, config.resolution))
                    pil.save(os.path.join(tmpdir, f"{idx}.jpg"))
                sampled_prompts = [samples["prompts"][ii] for ii in idxs]
                sampled_rewards = [samples["rewards"]['avg'][ii].item() for ii in idxs] if 'avg' in samples["rewards"] else [0.0]*len(idxs)
                wandb.log(
                    {"images": [wandb.Image(os.path.join(tmpdir, f"{idx}.jpg"),
        ...
        # per-prompt 统计
        if config.per_prompt_stat_tracking:
            prompt_ids = all_gather_concat(samples["prompt_ids"])
            prompts_all = tokenizers[0].batch_decode(prompt_ids.cpu().numpy(), skip_special_tokens=True)
            advantages = stat_tracker.update(prompts_all, rewards_np['avg'])
            ...
            adv_all = (rewards_np['avg'] - rewards_np['avg'].mean()) / (rewards_np['avg'].std() + 1e-4)
            advantages = torch.as_tensor(adv_all, device=device)
        
        advantages = torch.as_tensor(advantages)
        # 只保留本 rank 上对应切片（注意：这里我们简单使用全部 advantages 本地化，因为上面已经 all_gather；
        # 若你希望严格对应本 rank 样本，可基于 rank/世界大小切片。）
        samples["advantages"] = advantages.to(device)

        # 清理无用字段
        del samples["rewards"]; del samples["prompt_ids"]
        ...
        for index, prompt in enumerate(samples['prompts']):
            ...
            image_np = samples['images'][index].cpu().numpy().transpose(1, 2, 0) * 255
            ...
            with open(os.path.join(tmpdir, f"sample_{epoch}_{rank}_{index}.json"), "w", encoding="utf-8") as f:
                json.dump({"prompt": prompt, "advantage": advantage}, f, ensure_ascii=False, indent=2)
        ...
        if config.train.update_reward_model:
            ...
            print("rank ", rank, "batch_prompts: ", batch_prompts)
            print("rank ", rank, "reward batch has been created")
            print(f"rank {rank} , pos_imgs_B: ", len(pos_imgs_B))
            print(f"rank {rank} , pos_prompts_B: ", (pos_prompts_B))
            if len(pos_imgs_B) > 0:
            ...
            print(f"rank {rank} , reward_optimizer.step(), contrastive loss: ", float(contrastive_loss.item()))
            if is_main_process():
                wandb.log({"reward_contrastive_loss": float(contrastive_loss.item())}, step=global_step)
                # if epoch % 10 == 0:
                wandb.log(
                    {"positive_images": [wandb.Image(img, caption=pos_prompts_B[index_rm]) for index_rm, img in enumerate(pos_imgs_B)],
                        "negative_images": [wandb.Image(img, caption=neg_prompts_B[index_rm]) for index_rm, img in enumerate(neg_imgs_B)]},
                    step=global_step
                )
            reward_step += 1
